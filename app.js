...
// ── Naming conventions (applied in codegen only, not in the input field) ──
// Package: xlib prefix  →  xlibCustomerMgmt
// Procedure: _PC suffix →  processOrder_PC
// Function:  _FN suffix →  getCustomer_FN
// Record:    xr_ prefix →  xr_CustomerRec
// Table:     xt_ prefix →  xt_CustomerTab
// Cursor:    c_  prefix →  c_getOrders  (cfn already handles this)
function fqName(m) {
  const n = m.name || '';
  switch (m.type) {
    case 'PROCEDURE':   return n.endsWith('_PC')  ? n : n + '_PC';
    case 'FUNCTION':    return n.endsWith('_FN')  ? n : n + '_FN';
    case 'TYPE_RECORD': return n.startsWith('xr_') ? n : 'xr_' + n;
    case 'TYPE_TABLE':  return n.startsWith('xt_') ? n : 'xt_' + n;
    case 'CURSOR':      return cfn(n);
    default:            return n;
  }
}
// Package name: xlib prefix, no case forcing
function fqPkg(raw) {
  const n = raw || 'xlibUnnamed';
  return n.startsWith('xlib') ? n : 'xlib' + n.charAt(0).toUpperCase() + n.slice(1);
}
...
// ── Type tag HTML — shows the type label + convention hint ──
function typeTag(t) {
  ...
  const hint = { PROCEDURE:'+ _PC',  FUNCTION:'+ _FN',  TYPE_RECORD:'xr_ +', TYPE_TABLE:'xt_ +', CURSOR:'c_ +' };
  return `<span class="tag ${cls[t]}">${lbl[t]}<span class="tag-hint">${hint[t]}</span></span>`;
}
...
function methodCardHTML(m) {
  ...
  <div style="margin-top:10px">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:5px">
      ...
      <button class="btn-lc" onclick="addLC('${m.id}')">+ Add cursor</button>
    </div>
...
function generate() {
  const rawName = $('pkgName').value || '';
  const pkgName = fqPkg(rawName);
  ...
  const fqn     = schema ? `${schema}.${pkgName}` : pkgName;
...
function procComment(m) {
  const n = fqName(m);
  if (!S.procSig?.trim()) return m.purpose ? `  -- ${m.type}: ${n} | ${m.purpose}${S.author?' | '+S.author:''}\n` : '';
  return applyTpl(S.procSig, { name:n, type:m.type, date:new Date().toISOString().split('T')[0], author:S.author||'', purpose:m.purpose||'' }) + '\n';
}
...
// ── Keyboard shortcuts ────────────────────────────────
// Ctrl+Alt+Letter — disabled when focus is inside an editable field
const SHORTCUTS = {
  'p': () => addM('PROCEDURE'),
  'f': () => addM('FUNCTION'),
  'c': () => addM('CURSOR'),
...
document.addEventListener('keydown', e => {
  ...
  if (key === '?') { showShortcuts(); e.preventDefault(); return; }
